...
window.addEventListener("DOMContentLoaded", async () => {
    try{
        ...
        const getReportInfo = await axios.get('http://3.145.74.157:3000/premium/user/download/all', { headers: {"Authorization" : token} });
...
async function getLeaderBoardFromDB(){
    ...
    try{
        const getLeaderboard = await axios.get("http://3.145.74.157:3000/premium/allexpenses", { headers: {"Authorization" : token} });
...
async function downloadReport(){
    const response = await axios.get(`http://3.145.74.157:3000/premium/user/download`, { headers: {"Authorization" : token} });
